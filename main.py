...
def onmessage(update,bot:ObigramClient):
    try:
        ...
        if '/acc' in msgText:
            try:
                ...
                if getUser:
                    ...
                    jdb.save()
                    statInfo = infos.createStat(username,getUser,jdb.is_admin(username))
                    bot.sendMessage(update.message.chat.id,'Configuracion añadida📨')
            except:
                bot.sendMessage(update.message.chat.id,'❎Error en el comando❎')
            return
        if '/host' in msgText:
        ...
        if '/start' in msgText:
            start_msg = 'Bot: Uploader7.1.7\n'
            start_msg+= 'Dev: @anonedev\n'
            start_msg+= 'Para saber coml usar /tuto\n'
